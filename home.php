...
<div id="wrapper_outer">
	...
	<div class="top_right">
	  ...
	  <?php
		$query = 'SELECT * FROM items WHERE catid = 1 ;';
		$result = mysqli_query($db, $query);
	  ...
	  ?><div class="center_title_bar">Frequently bought Products</div>
	 <?php
		$query = 'select itemid, count(*) as c from orders group by itemid order by c desc limit 0 , 3 ;';
		$result = mysqli_query($db, $query);
		while($row = mysqli_fetch_assoc($result))
		{
			$q = 'select * from items where items.itemid='.$row['itemid'].';';
	  ...
	  <div class="prod_box">
        <div class="center_prod_box">
          <div class="product_title"><span><?php echo $r['name'] ;?></span></div>
          <div class="product_img"><span><img width="160" height="96" src=<?php echo '"'.$r['picurl'].'"'; ?> alt="" border="0" /></span></div>
          <div class="prod_price"><span class="reduce"><?php echo "Rs.".$r['price'] ; ?></span></div>
        </div>
        <div class="prod_details_tab"> <a href=<?php echo '"add.php?id='.$r['itemid'].'"'; ?> class="prod_buy">Add to Cart</a> </div>
      </div>
